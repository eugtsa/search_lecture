...
SCALE = 30
SPRITES = {'pac':'./images/pac.png','wall':'./images/wall.png','fin_pos':"./images/fin_pos.png"}
...
class WorldRenderSimple:
    ...
    def _draw_finish(self,score,map):
        GAME_FONT.render_to(SCREEN, ((map.size_x+0.5)*SCALE-5*SCALE, map.size_y*SCALE/2-SCALE), "FIN!", self.WHITE)
    ...
    def _draw_score(self,score,map:Map):
        GAME_FONT.render_to(SCREEN, (0, map.size_y*SCALE), "SCORE:{}".format(score), self.WHITE)

    def _draw_dot(self,dot_pos):
        pygame.draw.circle(SCREEN,self.YELLOW,(SCALE*dot_pos.x+SCALE/2,SCALE*dot_pos.y+SCALE/2),SCALE/4)
